...
def test_full_comsol_diagnostics_reads_synthetic_artifacts(tmp_path: Path) -> None:
    ...
    _write_json(
        solver_dir / "debug_diagnostics.json",
        {"collision": {"primary_hit_count": 1, "valid_mask_mixed_stencil_count": 2}},
    )
    _write_csv(
    ...
    _write_csv(
        solver_dir / "wall_events.csv",
        [{"particle_id": 2, "hit_time_s": 0.2, "part_id": 10}],
    )
    _write_json(
        first_step_dir / "first_step_summary.json",
        {
            "particle_count": 3,
    ...
    _write_csv(
        ...
        [
            ...
            {
                "particle_id": 2,
                "position_error_m": 2e-7,
                "velocity_error_mps": 0.02,
                "speed_ratio": 0.99,
            },
            {
                "particle_id": 3,
                "position_error_m": 3e-7,
                "velocity_error_mps": 0.03,
                "speed_ratio": 1.10,
            },
        ],
    )
    ...
    summary = json.loads(
        (output_dir / "full_comsol_diagnostics_summary.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    # Geometry proximity is optional unless debug diagnostics provide it.
    assert summary["near_wall_active"]["available"] == 0
    assert summary["near_wall_active"]["active_near_boundary_count"] is None
    ...
    assert (
        summary["runtime_collision_counters"]["counters"][
            "valid_mask_mixed_stencil_count"
        ]
        == 2
    )
    assert {
        item["source_part_id"] for item in summary["top_source_parts_for_residuals"]
    } == {10, 20}
    assert (output_dir / "suspicious_particles.csv").exists()


def test_full_comsol_diagnostics_reports_missing_optional_artifacts(
    tmp_path: Path,
) -> None:
    solver_dir = tmp_path / "solver"
    comsol_dir = tmp_path / "comsol"
    ...
    release = _write_csv(
        comsol_dir / "release.csv", [{"particle_id": 1, "source_part_id": 0}]
    )
    ...
    assert "debug_diagnostics" in summary["missing_optional_artifacts"]
    assert "wall_events" in summary["missing_optional_artifacts"]
