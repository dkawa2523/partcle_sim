param(
    ...
    [string]$Config = "external\comsol_particle_export\config\export_case.example.json",

    [Parameter(Mandatory=$false)]
    [string]$OutDir = "_external_exports\comsol_particle_case"
)
...
$javaFile = Join-Path $root "external\comsol_particle_export\java\ComsolParticleCaseExporter.java"
$classFile = Join-Path $root "external\comsol_particle_export\java\ComsolParticleCaseExporter.class"
...
$oldMph = $env:COMSOL_PARTICLE_MPH
$oldConfig = $env:COMSOL_PARTICLE_CONFIG
$oldOutDir = $env:COMSOL_PARTICLE_OUTDIR
try {
    $env:COMSOL_PARTICLE_MPH = [string]$mphPath
    $env:COMSOL_PARTICLE_CONFIG = [string]$configPath
    $env:COMSOL_PARTICLE_OUTDIR = [string]$outPath
    ...
    $env:COMSOL_PARTICLE_MPH = $oldMph
    $env:COMSOL_PARTICLE_CONFIG = $oldConfig
    $env:COMSOL_PARTICLE_OUTDIR = $oldOutDir
}

if (!(Test-Path (Join-Path $outPath "model_inventory.json"))) {
    throw "COMSOL export did not produce model_inventory.json. Check comsol_export.log in $outPath"
}

Write-Host "Raw COMSOL particle export written to $outPath"
