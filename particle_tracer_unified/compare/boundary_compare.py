...
ALIASES = {
    ...
    "part_id": ("part_id", "solver_part_id", "boundary_part_id", "comsol_entity_id", "entity_id", "boundary_id"),
...
def _numeric(frame: pd.DataFrame, canonical: str) -> pd.Series:
    col = _first_column(frame, ALIASES[canonical])
    if col is None:
        return pd.Series(np.nan, index=frame.index, dtype=float)
    return pd.to_numeric(frame[col], errors="coerce")


def _text(frame: pd.DataFrame, canonical: str) -> pd.Series:
    col = _first_column(frame, ALIASES[canonical])
    if col is None:
...
def _canonical_first_hits(frame: pd.DataFrame, *, label: str) -> pd.DataFrame:
    particle_col = _first_column(frame, ALIASES["particle_id"])
    time_col = _first_column(frame, ALIASES["hit_time_s"])
    if particle_col is None or time_col is None:
        raise ValueError(f"{label} boundary CSV must contain particle_id and hit_time_s/time_s")
    work = pd.DataFrame(
        {
            "particle_id": pd.to_numeric(frame[particle_col], errors="coerce"),
            "hit_time_s": pd.to_numeric(frame[time_col], errors="coerce"),
            "part_id": _numeric(frame, "part_id"),
            "outcome": _text(frame, "outcome"),
            "hit_x": _numeric(frame, "hit_x"),
            "hit_y": _numeric(frame, "hit_y"),
            "hit_z": _numeric(frame, "hit_z"),
            "normal_x": _numeric(frame, "normal_x"),
            "normal_y": _numeric(frame, "normal_y"),
            "normal_z": _numeric(frame, "normal_z"),
            "v_hit_x": _numeric(frame, "v_hit_x"),
            "v_hit_y": _numeric(frame, "v_hit_y"),
            "v_hit_z": _numeric(frame, "v_hit_z"),
        }
    )
    work = work[np.isfinite(work["particle_id"].to_numpy(dtype=float)) & np.isfinite(work["hit_time_s"].to_numpy(dtype=float))]
    if work.empty:
        return work
    ...
    counts = work.groupby("particle_id", as_index=False).size().rename(columns={"size": f"{label}_event_count"})
...
def _norm_error(frame: pd.DataFrame, bases: tuple[str, ...]) -> np.ndarray:
    ...
    for base in bases:
        ...
        if left in frame.columns and right in frame.columns:
            delta = pd.to_numeric(frame[left], errors="coerce") - pd.to_numeric(frame[right], errors="coerce")
...
def _summary(merged: pd.DataFrame) -> dict[str, object]:
    ...
    out: dict[str, object] = {
        "particle_count": int(len(merged)),
        "matched_first_hit_count": int(np.count_nonzero(both.to_numpy())),
        "python_only_count": int(np.count_nonzero((merged["_merge"] == "left_only").to_numpy())),
        "comsol_only_count": int(np.count_nonzero((merged["_merge"] == "right_only").to_numpy())),
    }
    for name in ("hit_time_error_s", "hit_position_error_m", "normal_error", "hit_velocity_error_mps"):
        if name not in merged.columns:
            continue
        values = pd.to_numeric(merged.loc[both, name], errors="coerce").to_numpy(dtype=float)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compare first wall-hit diagnostics against COMSOL boundary-hit CSV.")
    parser.add_argument("--python", dest="python_csv", required=True, type=Path)
    parser.add_argument("--comsol", required=True, type=Path)
    parser.add_argument("--output", "--out", dest="output", type=Path, default=Path("boundary_hit_comparison.csv"))
    ...
    merged = python_df.merge(comsol_df, on=["particle_id"], how="outer", suffixes=("_python", "_comsol"), indicator=True)
    ...
    for base in ("part_id", "outcome"):
        ...
        if left in merged.columns and right in merged.columns:
            merged[f"{base}_match"] = merged[left].astype(str) == merged[right].astype(str)
    ...
    merged["hit_velocity_error_mps"] = _norm_error(merged, ("v_hit_x", "v_hit_y", "v_hit_z"))
    write_csv(merged, args.output)
    if args.summary is not None:
        args.summary.parent.mkdir(parents=True, exist_ok=True)
        args.summary.write_text(json.dumps(_summary(merged), indent=2) + "\n", encoding="utf-8")
    return 0
